// Seed script: Insert sample calculation data for Songkhla & Hat Yai
// Run: npx ts-node prisma/seed-locations.ts
// Or:  npx tsx prisma/seed-locations.ts
...
async function main() {
    console.log('🌱 Seeding sample calculation data for Songkhla & Hat Yai...\n');

    const records = [];

    for (const loc of LOCATIONS) {
        // Each location gets 1-4 calculations
    ...
    for (const record of records) {
        await db.calculation.create({ data: record });
    }

    console.log(`✅ Inserted ${records.length} sample calculations`);
    console.log(`   📍 Locations: ${LOCATIONS.length}`);
    console.log(`   🧪 Chemicals: ${CHEMICALS.map(c => c.name).join(', ')}`);
